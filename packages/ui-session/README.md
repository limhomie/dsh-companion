...
Session Route 使用容量为 3 的内存 LRU 保留最近访问的完整视图树。切回缓存项会复用已渲染 Conversation、滚动位置和组件私有草稿；隐藏项通过 `hidden` 离开可访问性树并暂停 `SessionFace` 订阅，重新激活时直接读取同一个 Harness Runtime Snapshot。淘汰、Host 列表删除或 Route 卸载会释放视图及其监听器。该缓存不保存第二份 Conversation 状态、不持久化正文，也不替代首次打开时的 Host 历史基线。

对话 Renderer 复用 Harness `ui-primitives` 的 Markdown 组件展示 Agent 正文，用户输入保持字面文本并使用右对齐气泡。Reasoning 与 Tool 调用显示为可展开轨迹行；已有结果的 Tool 调用只显示一次，折叠态保留名称、主要参数和完成状态。底部 Composer 把文本区、Host 命令、权限、模型、推理强度、队列状态与发送或停止操作放在同一个容器中；展开操作只改变这个组件的覆盖布局，不复制草稿、菜单或 Operation id。权限读取 Session `permissions` Projection 并通过 `/permission` 写回；模型目录与推理强度读取 `session.models` 并通过 `session.selectModel` 写回；命令菜单每次从 Host `command.list` 加载。Agent preset 目录是可安全重试的只读请求，使用有界超时和一次重试；Session 创建超时后的显式重试依赖 Runtime 合并同一 Workspace 在途事务。命令、权限和模型菜单在点击控制区外或按下 Escape 后关闭；全屏状态下第二次 Escape 收起编辑器。附件仍未接入。
